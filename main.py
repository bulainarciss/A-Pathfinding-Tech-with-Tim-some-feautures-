...
class Spot:
    ...
    def update_neighbors(self, grid):
        ...
        if self.row < self.total_rows - 1 and self.col < self.total_rows - 1 and \
                not grid[self.row + 1][self.col + 1].is_barrier():  # Down
            self.neighbors.append(grid[self.row + 1][self.col + 1])

        if self.row > 0 and self.col > 0 and \
                not grid[self.row - 1][self.col - 1].is_barrier():  # Up
            self.neighbors.append(grid[self.row - 1][self.col - 1])

        if self.col < self.total_rows - 1 and self.row > 0 and \
                not grid[self.row - 1][self.col + 1].is_barrier():  # Right
            self.neighbors.append(grid[self.row - 1][self.col + 1])

        if self.col > 0 and self.row < self.total_rows - 1 and \
                not grid[self.row + 1][self.col - 1].is_barrier():  # Left
            self.neighbors.append(grid[self.row + 1][self.col - 1])
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                ...
                if event.key == pygame.K_r:
                    for row in grid:
                        for spot in row:
                            if spot.color == PURPLE or spot.color == GREEN or spot.color == RED:
                                spot.reset()
                if event.key == pygame.K_j:
                    for row in grid:
                        for spot in row:
                            if spot.color == GREEN or spot.color == RED:
                                spot.reset()
                if event.key == pygame.K_g:
                    for row in grid:
                        for spot in row:
                            x = random.randint(1, 100000)
                            if x % 2 == 0 and spot != start and spot != end:
                                spot.make_barrier()
